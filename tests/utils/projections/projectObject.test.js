describe('Object list projection', () => {
  const mockDataArray = [
    { food: 'mhajeb', drink: 'lben', isBnin: true },
    { food: 'zviti', drink: 'rayeb', isBnin: true },
  ];
  ...
  describe('if no projection parameter is passed', () => {
    it.each([
      ...
      'should return the "same" data %s as it is if no parameters were passed',
      (type, data) => {
  ...
  describe('when passing the projection parameter', () => {
    ...
    it('should return the data object with only the desired attributes', () => {
      expect(projectObject(mockDataObject, projection)).toEqual({
        food: 'karentika',
        isBnin: true,
      });
    });

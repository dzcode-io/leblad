...
const ALGERIA_API_RELEASES_URL =
  'https://api.github.com/repos/fcmam5/algeria-api/releases/latest';
...
(async () => {
  ...
  const currentVersion = await readFileAsync(VERSION_FILE_PATH, {
    encoding: 'utf8',
  });
  ...
  if (currentVersion === tagName) {
    ...
    console.warn(
      `⚠️ Another dataset version is present! Gonna update from "${currentVersion}" to "${tagName}"`,
    );

    // Write data to data/wilayalist.json
    const wilayaListJsonFileUrl = assets.find(
      (asset) => asset.name === WILAYA_LIST_MINIFIED_JSON_FILE_NAME,
    ...
    wilayaListJsonFileResponse.data.pipe(
      fs.createWriteStream(`${DATA_DIRECTORY}/${WILAYA_LIST_JSON_FILE_NAME}`),
    );
